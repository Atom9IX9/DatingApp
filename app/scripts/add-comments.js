...
function describeFromPath(filePath) {
  ...
  if (parts.includes("api"))
    return `API definitions for ${fileName} in the ${parts.join(" > ")} area.`;
  if (parts.includes("hooks"))
    return `Reusable hook definitions for ${fileName}.`;
  if (parts.includes("ui")) return `UI component definitions for ${fileName}.`;
  if (parts.includes("lib"))
    return `Library helper code or utilities for ${fileName}.`;
...
function descriptionForDeclaration(declName, filePath, trimmedLine) {
  ...
  if (declName === "k")
    return "Scale factor used to adjust avatar image position and size based on the rendered component dimensions.";
  if (declName === "PosAvatar")
    return "React component that renders an avatar crop area using transform and clipping.";
  if (declName === "Props") return "Type describing component props.";
  if (declName.endsWith("Props"))
    return `Props type for the ${declName.slice(0, -5)} component.`;
  if (declName.endsWith("State"))
    return `Shape of the ${declName.slice(0, -5)} state.`;
  if (declName.startsWith("use"))
    return `Custom hook that manages ${declName.slice(3)} logic.`;
  if (declName.endsWith("Provider"))
    return `Provider that supplies ${declName.slice(0, -8)} context or state.`;
  if (declName.endsWith("Slice"))
    return `Redux slice that manages ${declName.slice(0, -5)} state.`;
  if (declName.endsWith("API"))
    return `API helper for ${declName.slice(0, -3)} operations.`;
  if (declName.endsWith("Form"))
    return `Form component that captures ${declName
      .slice(0, -4)
      .replace(/([A-Z])/g, " $1")
      .trim()
      .toLowerCase()} input.`;
  if (declName.endsWith("Button") || declName.endsWith("Btn"))
    return `Button component for a user action in ${path.relative(root, filePath)}.`;
  if (declName.endsWith("Menu"))
    return `Menu component or interaction handler.`;
  if (declName.endsWith("Page"))
    return `Page component for the ${declName.slice(0, -4)} view.`;
  if (declName === "middleware")
    return "Middleware function that inspects requests and applies redirects or headers.";
  if (declName === "config")
    return "Configuration object used by middleware or routing.";
  if (lower.startsWith("validate"))
    return `Validation helper that checks whether ${declName.slice(8).toLowerCase()} input is valid.`;
  if (lower.startsWith("get"))
    return `Helper that returns computed data for ${declName.slice(3).toLowerCase()}.`;
  if (lower.startsWith("set"))
    return `Helper that updates values or state for ${declName.slice(3).toLowerCase()}.`;
  if (lower.startsWith("is"))
    return `Boolean helper that checks whether ${declName.slice(2).toLowerCase()} is true.`;
  if (lower.startsWith("fetch") || lower.startsWith("load"))
    return `Function that loads data for ${lower.slice(lower.startsWith("fetch") ? 5 : 4)}.`;
  if (declName.startsWith("create"))
    return `Factory helper that creates ${declName.slice(6).toLowerCase()} objects or state.`;
  if (declName.startsWith("make"))
    return `Utility that builds or initializes ${declName.slice(4).toLowerCase()} values.`;
  if (declName === "Startpage")
    return "Server-rendered page component for the start page.";
  if (declName === "RootLayout")
    return "Root layout component that wraps every page and reads server cookies/headers.";
  if (declName === "verifyAuth")
    return "Async function that validates the auth token by calling the backend.";
  return null;
}
function commentForLine(trimmed) {
  if (
    ...
    trimmed.startsWith("// Client component") ||
    trimmed.startsWith("// Server component") ||
  ...
  const reactState = trimmed.match(
    /^(const|let|var)\s+\[([A-Za-z0-9_]+),\s*set([A-Z][A-Za-z0-9_]*)\]\s*=\s*useState\(/,
  );
  if (reactState) {
  ...
  if (
    /\bawait\s+fetch\(/.test(trimmed) ||
    /^const\s+[A-Za-z0-9_]+\s*=\s*fetch\(/.test(trimmed)
  ) {
...
function getFileVariantComments(filePath, lines) {
  ...
  if (hasUseClient(lines)) {
    result.push(
      "// Client component: this file runs in the browser and can use hooks and DOM APIs.",
    );
  } else if (isServerComponent(filePath, lines)) {
    result.push(
      "// Server component (SSR): this Next.js file is rendered on the server.",
    );
  }
  return result;
}
...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const namedMatch = trimmed.match(
      /^(const|function|class)\s+([A-Za-z0-9_]+)/,
    );
    let declName = null;
    if (declMatch) declName = declMatch[3];
    else if (defaultMatch) declName = defaultMatch[1];
    else if (namedMatch && trimmed.startsWith("const") && trimmed.includes("="))
      declName = namedMatch[2];
    const isDefaultExportStatement =
